...
mc::BaseConnection *mc::ConnectionStatus::handle_packet(mc::Varint::Int packet_id, mc::Buffer &packet,
                                                        mc::PacketClientBound **response) {
    switch (packet_id) {
        case 0x00: {
            // empty request
            auto empty(read_inbound_packet<PacketEmpty>(packet));

            auto json = status_json();
            *response = new PacketStatusResponse;
            (*response)->get_field_value<String>("status") = String(json.size(), json.c_str());
            return this;
        }
        case 0x01: {
            // ping
            auto ping(read_inbound_packet<PacketPing>(packet));

            *response = new PacketPong;
            (*response)->get_field_value<Long>("payload") = ping.get_field_value<Long>("payload");
...
static std::string status_json() {
    return "{\"description\":{\"text\":\"My Nice Server\"},\"players\":{\"max\":800,\"online\":409},"
